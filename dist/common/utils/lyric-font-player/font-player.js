...
module.exports = class FontPlayer {
    constructor({ time = 0, rate = 1, lyric = '', lineContentClassName = 'line-content', lineClassName = 'line', shadowClassName = 'shadow', fontModeClassName = 'font-mode', lineModeClassName = 'line-mode', fontLrcClassName = 'font-lrc', extendedLrcClassName = 'extended', shadowContent = false, extendedLyrics = [], isVertical = false, }) {
        this.time = time;
        this.lyric = lyric;
        this._rate = rate;
    ...
    _parseLyric() {
        ...
        for (const font of fonts) {
            if (!timeRxp.test(font))
                return this._handleLineParse();
    ...
    _refresh() {
        ...
        if (this.curFontNum >= this.maxFontNum)
            return this._handlePlayMaxFontNum();
        let curFont = this.fonts[this.curFontNum];
    ...
    play(curTime = 0) {
        ...
        this.pause();
        if (this.isLineMode)
            return this._handlePlayLine(true);
        this.lineContent.classList.remove('played');
        this.isPlay = true;
    ...
    pause() {
        ...
        if (this.curFontNum === this.maxLine)
            return;
        const curFontNum = this._findcurFontNum(this._currentTime());
        if (this.curFontNum === curFontNum)
            return;
        for (let i = 0; i < this.curFontNum; i++)
            this._handlePlayFont(this.fonts[i], 0, true);
    }
    ...
    reset() {
        this.pause();
        if (this.isLineMode)
            return this._handlePlayLine(false);
        this.lineContent.classList.remove('played');
        for (const font of this.fonts) {
            font.animation.cancel();
            font.dom.style.backgroundSize = '0 100%';
