...
module.exports = class Lyric {
    constructor({ lyric = '', extendedLyrics = [], offset = 0, rate = 1, lineContentClassName = 'line-content', lineClassName = 'line', shadowClassName = 'shadow', fontModeClassName = 'font-mode', lineModeClassName = 'line-mode', fontLrcClassName = 'font-lrc', extendedLrcClassName = 'extended', activeLineClassName = 'active', shadowContent = false, isVertical = false, onPlay = function (line, text) { }, onSetLyric = function (lines, offset) { }, onUpdateLyric = function (lines) { }, }) {
        this.lyric = lyric;
    ...
    _initLines = (lyricLines, offset, isUpdate) => {
        ...
        if (isUpdate)
            this.onUpdateLyric(this._lines);
        else
    ...
    pause() {
        ...
        if (this.playingLineNum > -1)
            this._lineFonts[this.playingLineNum]?.pause();
    }
    setOffset(offset) {
