...
var _require2 = require('@common/rendererIpc'), rendererInvoke = _require2.rendererInvoke;
var _require3 = require('@common/ipcNames'), WIN_MAIN_RENDERER_EVENT_NAME = _require3.WIN_MAIN_RENDERER_EVENT_NAME;
...
var parseTools = {
    ...
    parse: function parse(lrc, tlrc, rlrc) {
        ...
        if (lrc) {
            var _this$parseLyric = this.parseLyric(this.removeTag(lrc)), lyric = _this$parseLyric.lyric, lxlyric = _this$parseLyric.lxlyric;
            info.lyric = lyric;
        ...
        if (rlrc)
            info.rlyric = this.fixRlrcTimeTag(this.parseRlyric(this.removeTag(rlrc)), info.lyric);
        if (tlrc)
            info.tlyric = this.fixTlrcTimeTag(tlrc, info.lyric);
        ...
        return {
            ...
            promise: this.getSongId(mInfo).then(function (songId) {
                ...
                return requestObj.promise.then(function (_ref2) {
                    var body = _ref2.body;
                    // console.log(body)
                    if (body.code != _this4.successCode || body.req.code != _this4.successCode)
                        return _this4.getLyric(songId, ++retryNum);
