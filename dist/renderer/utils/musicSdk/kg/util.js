...
exports.signatureParams = exports.decodeLyric = exports.createHttpFetch = void 0;
...
var _require = require('zlib'), inflate = _require.inflate;
var _require2 = require('../utils'), toMD5 = _require2.toMD5;
var _require3 = require('../../request'), httpFetch = _require3.httpFetch;
// https://github.com/lyswhut/lx-music-desktop/issues/296#issuecomment-683285784
var enc_key = Buffer.from([0x40, 0x47, 0x61, 0x77, 0x5e, 0x32, 0x74, 0x47, 0x51, 0x36, 0x31, 0x2d, 0xce, 0xd2, 0x6e, 0x69], 'binary');
var decodeLyric = function decodeLyric(str) {
    return new Promise(function (resolve, reject) {
        if (!str.length)
            return;
        var buf_str = Buffer.from(str, 'base64').slice(4);
        for (var i = 0, len = buf_str.length; i < len; i++) {
            buf_str[i] = buf_str[i] ^ enc_key[i % 16];
        }
        inflate(buf_str, function (err, result) {
            if (err)
                return reject(err);
...
var signatureParams = function signatureParams(params) {
    var apiver = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 9;
    var keyparam = 'OIlwieks28dk2k092lksi2UIkp';
    if (apiver === 5)
        keyparam = 'NVPh5oo715z5DIWAeQlhMDsWXXQV4hwt';
    var param_list = params.split('&');
    param_list.sort();
    var sign_params = "".concat(keyparam).concat(param_list.join('')).concat(keyparam);
...
var createHttpFetch = /*#__PURE__*/ function () {
            ...
            while (1)
                switch (_context.prev = _context.next) {
                    ...
                    case 9:
                        ...
                        return _context.abrupt("return", createHttpFetch(url, options, ++retryNum));
                    case 13:
                        if (!(result.statusCode !== 200 || ((_ref2 = (_result$body$error_co = result.body.error_code) !== null && _result$body$error_co !== void 0 ? _result$body$error_co : result.body.errcode) !== null && _ref2 !== void 0 ? _ref2 : result.body.err_code) != 0)) {
                            _context.next = 15;
                            break;
                        }
                        return _context.abrupt("return", createHttpFetch(url, options, ++retryNum));
                    case 15:
                        ...
                        return _context.abrupt("return", result.body.data);
                    case 17:
                    ...
                    case 19:
                        return _context.abrupt("return", result.body.info);
                    case 20:
                    case "end":
    ...
    return function createHttpFetch(_x, _x2) {
