...
exports.tokenRequest = exports.objStr2JSON = exports.matchToken = exports.lrcTools = exports.getToken = exports.formatSinger = exports.decodeLyric = void 0;
...
var _require = require('../../request'), httpGet = _require.httpGet, httpFetch = _require.httpFetch;
var _require2 = require('@common/ipcNames'), WIN_MAIN_RENDERER_EVENT_NAME = _require2.WIN_MAIN_RENDERER_EVENT_NAME;
var _require3 = require('@common/rendererIpc'), rendererInvoke = _require3.rendererInvoke;
var kw_token = {
    token: null,
    isGetingToken: false
};
...
// }
var objStr2JSON = function objStr2JSON(str) {
    return JSON.parse(str.replace(/('(?=(,\s*')))|('(?=:))|((?<=([:,]\s*))')|((?<={)')|('(?=}))/g, '"'));
};
exports.objStr2JSON = objStr2JSON;
var formatSinger = function formatSinger(rawData) {
    return rawData.replace(/&/g, '、');
};
exports.formatSinger = formatSinger;
var matchToken = function matchToken(headers) {
    try {
...
var getToken = function getToken() {
    var retryNum = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    return new Promise(function (resolve, reject) {
        if (retryNum > 2)
            return Promise.reject(new Error('try max num'));
        if (kw_token.isGetingToken)
            return wait(1000).then(function () {
                return getToken(retryNum).then(function (token) {
                    return resolve(token);
                });
            });
        if (kw_token.token)
            return resolve(kw_token.token);
        kw_token.isGetingToken = true;
        httpGet('http://www.kuwo.cn/', function (err, resp) {
            kw_token.isGetingToken = false;
            if (err)
                return getToken(++retryNum);
            if (resp.statusCode != 200)
                return reject(new Error('获取失败'));
            var token = kw_token.token = matchToken(resp.headers);
            resolve(token);
        });
    });
};
exports.getToken = getToken;
var decodeLyric = function decodeLyric(base64Data) {
    return rendererInvoke(WIN_MAIN_RENDERER_EVENT_NAME.handle_kw_decode_lyric, base64Data);
};
exports.decodeLyric = decodeLyric;
var tokenRequest = /*#__PURE__*/ function () {
    var _ref = _asyncToGenerator(/*#__PURE__*/ _regeneratorRuntime().mark(function _callee(url) {
        var options, token, requestObj, _args = arguments;
            ...
            while (1)
                switch (_context.prev = _context.next) {
                    ...
                    case 6:
                        ...
                        requestObj = httpFetch(url, options);
                        requestObj.promise = requestObj.promise.then(function (resp) {
                            // console.log(resp)
                            if (resp.statusCode == 200) {
    ...
    return function tokenRequest(_x) {
...
var lrcTools = {
    ...
    lines: [],
    tags: [],
    getWordInfo: function getWordInfo(str, str2, prevWord) {
        var offset = parseInt(str);
        var offset2 = parseInt(str2);
        var startTime = Math.abs((offset + offset2) / (this.offset * 2));
        var endTime = Math.abs((offset - offset2) / (this.offset2 * 2)) + startTime;
        if (prevWord) {
            if (startTime < prevWord.endTime) {
                ...
                prevWord.newTimeStr = "<".concat(prevWord.startTime, ",").concat(prevWord.endTime - prevWord.startTime, ">");
        ...
        return {
            startTime: startTime,
            endTime: endTime,
            timeStr: "<".concat(startTime, ",").concat(endTime - startTime, ">")
        };
    },
    parseLine: function parseLine(line) {
        if (line.length < 6)
            return;
        var result = this.rxps.wordLine.exec(line);
        if (result) {
            var time = result[1];
            var words = result[2];
            if (words == null) {
            ...
            var wordTimes = words.match(this.rxps.wordTimeAll);
            if (!wordTimes)
                return;
            // console.log(wordTimes)
            var preTimeInfo;
            var _iterator = _createForOfIteratorHelper(wordTimes), _step;
            try {
                for (_iterator.s(); !(_step = _iterator.n()).done;) {
                    var _preTimeInfo;
                    var timeStr = _step.value;
                    var _result = this.rxps.wordTime.exec(timeStr);
                    var wordInfo = this.getWordInfo(_result[1], _result[2], preTimeInfo);
                    words = words.replace(timeStr, wordInfo.timeStr);
                    if ((_preTimeInfo = preTimeInfo) !== null && _preTimeInfo !== void 0 && _preTimeInfo.newTimeStr)
        ...
        if (result[1] == 'kuwo') {
            ...
            var _valueOf = parseInt(content, 8);
            this.offset = Math.trunc(_valueOf / 10);
            this.offset2 = Math.trunc(_valueOf % 10);
    ...
    parse: function parse(lrc) {
        // console.log(lrc)
        var lines = lrc.split(/\r\n|\r|\n/);
        var tools = Object.create(this);
        tools.isOK = true;
        tools.offset = 1;
        tools.offset2 = 1;
        tools.lines = [];
        tools.tags = [];
        var _iterator2 = _createForOfIteratorHelper(lines), _step2;
        try {
        ...
        if (!tools.lines.length)
            return '';
        var lrcs = tools.lines.join('\n');
        if (tools.tags.length)
            lrcs = "".concat(tools.tags.join('\n'), "\n").concat(lrcs);
        // console.log(lrcs)
        return lrcs;
    }
};
