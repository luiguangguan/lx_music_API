...
var _require2 = require('./musicInfo'), _getMusicInfo = _require2.getMusicInfo;
var _require3 = require('./utils/mrc'), decrypt = _require3.decrypt;
var mrcTools = {
    rxps: {
        lineTime: /^\s*\[(\d+),\d+\]/,
        wordTime: /\(\d+,\d+\)/,
    ...
    getLyric: function getLyric(songInfo) {
        ...
        return {
            promise: this.getMusicInfo(songInfo).then(function (info) {
                var p;
                if (info.mrcUrl)
                    p = _this4.getMrc(info.mrcUrl);
                else if (info.lrcUrl)
                    p = _this4.getLrc(info.lrcUrl);
                if (p == null)
                    return Promise.reject('获取歌词失败');
                return Promise.all([p, _this4.getTrc(info.trcUrl)]).then(function (_ref2) {
                    var _ref3 = _slicedToArray(_ref2, 2), lrcInfo = _ref3[0], tlyric = _ref3[1];
    ...
    getLyricWeb: function getLyricWeb(songInfo) {
        ...
        if (songInfo.lrcUrl) {
            ...
            requestObj.promise = requestObj.promise.then(function (_ref4) {
                var body = _ref4.body, statusCode = _ref4.statusCode;
                if (statusCode !== 200) {
                    if (tryNum > 5)
                        return Promise.reject('歌词获取失败');
