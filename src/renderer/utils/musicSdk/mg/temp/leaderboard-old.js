...
// const { sizeFormate } = require('../index');
...
var _default = {
  ...
  getUrl: function getUrl(id, page) {
    var targetBoard = boardList.find(function (board) {
      return board.bangid == id;
    });
    return "https://music.migu.cn/v3/music/top/".concat(targetBoard.webId);
  ...
  formateIntv: function formateIntv() {},
  filterData: function filterData(rawData) {
    ...
    rawData.forEach(function (item) {
      ...
      list.push({
        singer: _this.getSinger(item.singers),
        name: item.name,
        ...
        source: 'mg',
        interval: item.duration ? formatPlayTime(_this.getIntv(item.duration)) : null,
        img: item.mediumPic ? "https:".concat(item.mediumPic) : null,
        lrc: null,
        // lrcUrl: item.lrcUrl,
        otherSource: null,
        types: types,
  ...
  filterBoardsData: function filterBoardsData(rawList) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        var _iterator2 = _createForOfIteratorHelper(board.itemList),
          _step2;
        try {
  ...
  getList: function getList(bangid, page) {
    ...
    return this.getData(this.getUrl(bangid, page)).then(function (_ref) {
      var statusCode = _ref.statusCode,
        body = _ref.body;
      if (statusCode !== 200) return _this3.getList(bangid, page, retryNum);
      var listData = body.match(_this3.regExps.listData);
      if (!listData) return _this3.getList(bangid, page, retryNum);
      var datas = JSON.parse(RegExp.$1);
      // console.log(datas)
      listData = _this3.filterData(datas.songs.items);
      return {
        total: datas.songs.itemTotal,
        list: _this3.filterData(datas.songs.items),
        limit: _this3.limit,
        page: page,
        source: 'mg'
      };
