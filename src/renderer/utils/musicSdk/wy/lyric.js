...
var parseTools = {
  ...
  parseHeaderInfo: function parseHeaderInfo(str) {
    ...
    return lines.map(function (line) {
      if (!_this2.rxps.info.test(line)) return line;
      try {
  ...
  parse: function parse(ylrc, ytlrc, yrlrc, lrc, tlrc, rlrc) {
    ...
    if (lrc) {
      ...
      if (_lines3) info.lyric = _lines3.join('\n');
    }
    if (tlrc) {
      var _lines4 = this.parseHeaderInfo(tlrc);
      if (_lines4) info.tlyric = _lines4.join('\n');
    }
    if (rlrc) {
      var _lines5 = this.parseHeaderInfo(rlrc);
      if (_lines5) info.rlyric = _lines5.join('\n');
    }
    return info;
  }
};
...
var _default = function _default(songmid) {
  ...
  requestObj.promise = requestObj.promise.then(function (_ref) {
    ...
    var body = _ref.body;
    // console.log(body)
    if (body.code !== 200 || !(body !== null && body !== void 0 && (_body$lrc = body.lrc) !== null && _body$lrc !== void 0 && _body$lrc.lyric)) return Promise.reject(new Error('Get lyric failed'));
